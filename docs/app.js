...
const elFinish = $                   ("finish");   // the SAVE key (banks to history)
const elReset = $                   ("reset");     // the DISCARD key (throws the run away)
...
function setLamps(state        , locked         , alarmStyle         ) {
  ...
  const alarmTxt = ({ blip: "♪BLIP", digital: "♪DIGITAL", bell: "♪BELL", melody: "♪MELODY", silent: "ALARM OFF" }                          )[alarmStyle ?? ""] ?? "♪CHIME";
...
function bellAlarm() {
  ...
  const partials                          = [[1568, 0.22], [2489, 0.12], [3951, 0.06]];
...
// ── render ──────────────────────────────────────────────────────
async function renderDevice(pre           )                    {
...
function renderLauncher() {
  ...
  const bay = (cls        , glyph        , label        , jp        , onClick            ) => {
    const li = el("li", `timer-row launch ${cls}`)                 ;
...
function operable                       (node   , label        , fn            )    {
  node.tabIndex = 0;
...
function refreshBanked(sessions           , now        ) {
...
function cardItem(c      , active               , index        , sessions           , now        , timers         )                {
  ...
  const flip = el("button", "card-flip", "⟲")                     ;
  flip.title = "flip ・ 裏面";
  flip.setAttribute("aria-label", `flip ${c.name} over to its record`);
  flip.onclick = (e) => {
    e.stopPropagation();
    const cardEl = (e.currentTarget               ).closest(".pocket")?.querySelector(".card")                      ;
...
function offerUndo(message        , restore                     ) {
  ...
  elUndo.innerHTML = "";
  const btn = el("button", "undo-key", "UNDO ・ 取消")                     ;
  btn.type = "button";
  btn.onclick = async () => {
    clearTimeout(undoTimer);
    elUndo.hidden = true;
...
async function deleteCardWithUndo(c      ) {
...
async function deleteTimerWithUndo(t       ) {
  ...
  offerUndo(`deleted timer "${t.name}"`, async () => {
    ...
    const restored        = { ...t, liveSession: null };
...
function setFaceVisibility(card3d         , showingBack         ) {
...
function openEditor(dlg                   ) { dlg.showModal(); sndSheet(1); }
async function closeEditor(dlg                   ) {
  const card = dlg.querySelector(".index-card")               ;
  sndSheet(-1);
  if (!reducedMotion()) {
...
$("c-emblem-chips").addEventListener("click", (e) => {
  ...
  if (b) { sndTick(); $                  ("c-emblem").value = b.textContent ?? ""; }
});
...
let editMode            = "up";
let editAlarm             = "chime";
const ALARM_CYCLE               = ["chime", "blip", "digital", "bell", "melody", "silent"];
const ALARM_LABEL                         = {
...
const UNIT = { h: { el: elH, max: 23 }, m: { el: elM, max: 59 }, s: { el: elS, max: 59 } }         ;
...
for (const btn of document.querySelectorAll                   (".re-step")) {
  const u = btn.dataset.u                     , d = Number(btn.dataset.d);
...
let rackReturnFocus                     = null;

function openTimerEditor(cardId        , timer              , opts                                                   = {}) {
  ...
  rackReturnFocus = document.activeElement                      ;
...
function closeTimerEditor() {
  ...
  queueMicrotask(() => {
    ...
    else (elTimerList.querySelector(".timer-row.add, .t-nm")                      )?.focus();
  });
}
...
$("t-save").onclick = async () => {
  ...
  const mode            = targetMs > 0 ? "down" : editMode === "down" ? "down" : "up";
...
$("sb-import-key").onclick = () => { sndClick(); $                  ("sb-import").click(); };
$                  ("sb-import").onchange = async (e) => {
  ...
  try {
    ...
    ($("sb-import")                    ).value = "";   // let the same file be picked again
  }
};
$("sb-clear").onclick = () => {
